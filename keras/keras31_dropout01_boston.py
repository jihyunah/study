...
from tensorflow.keras.layers import Dense, Input, Dropout  #함수형에는 인풋 레이어 해야함. 
import numpy as np
...
model.add(Dropout(0.5)) # --> 위의 레이어에서 반만큼 노드를 빼주겠다.
model.add(Dense(40, activation='sigmoid'))
model.add(Dropout(0.3)) # --> 위의 레이어에서 0.3만큼 노드를 빼주겠다.
model.add(Dense(30, activation='relu'))
model.add(Dropout(0.2)) # --> 위의 레이어에서 0.2만큼 노드를 빼주겠다.
